...
public class ReadExeclSheet {
	...
	public List<TableInfo> readTableList (Workbook  workbook, String sheetName ) {
		...
		if (readsheet == null ){
			logger.error("Sheet is " + readsheet + " 获取 Sheet 出错：" + sheetName );
			return null;
		}
...
//				Cell cell = row.getCell(j);
		...
		logger.debug("开始读取" + tabEnName + " 数据");
		...
		for (int i=baserow+1; i<=rowNum; i++){
			...
			else if ("索引信息".equals(row.getCell(enNameCol).getStringCellValue())|| "索引名".equals(row.getCell(enNameCol).getStringCellValue())) {
				break;
			}
		...
		logger.info("读 " + tabEnName + " : " + tabChName + "表结构详细信息   完成 ");
		return dataLineList;
	}
