...
//			logger.debug("当前处理行为:" + row.getCell(seqCol) );
			...
			tbInfo.setTabEnName(ReadExcelUtil.getCellString(row, tabEnCol) );
	...
	public List<DataLineInfo> readTableDef (Workbook  workbook, String InSheetName , String tabEnName, String tabChName ) {
		//logger.info("读 " + sheetName + "表结构详细信息  开始 ");
		//Sheet readsheet = workbook.getSheet(sheetName);
...
//					Cell cell = row.getCell(j);
				...
				if(content.contains("字段英文名")){
					...
					baseCol = j;
					enNameCol = j;
				}else if (content.contains("字段英文名")){
					enNameCol = j;
				}else if (content.contains("字段中文名")){
					chNameCol = j;
				}else if (content.contains("字段类型")){
					typeCol = j;
				}else if (content.contains("是否允许为空")){
					isNullCol = j;
				}else if (content.contains("主键")){
					isPKCol = j;
				}else if (content.contains("外键")){
					isFKCol = j;
				}else if (content.contains("缺省值")){
					defaultValueCol = j;
				}else if (content.contains("字段说明")){
					descCol = j;
				}else if (content.contains("数据标准编号")){
					dataStandardNoCol = j;
				}else if (content.contains("数据标准中文名称")){
		...
		logger.debug("开始读取" + sheetName + " 数据");
		for (int i=baserow+1; i<=rowNum; i++){
			DataLineInfo dataLine = new DataLineInfo();
			Row row = readsheet.getRow(i);
			if(row == null){
				continue;
			}
			else if ("索引名".equals(row.getCell(enNameCol).getStringCellValue())) {
				break;
			}
			logger.debug(sheetName + " 当前处理行为:" + i + " :" + row.getOutlineLevel());
			dataLine.setSeqNo(i - baserow );              
			dataLine.setEnName          (ReadExcelUtil.getCellString(row, enNameCol));
			dataLine.setChName          (ReadExcelUtil.getCellString(row, chNameCol));
			dataLine.setType            (ReadExcelUtil.getCellString(row, typeCol  ));
		...
		logger.info("读 " + sheetName + "表结构详细信息  完成 ");
		return dataLineList;
	}
